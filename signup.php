<?php 
		...
		if(isset($data['do_signup']))
		{
			...
			if ( R::count('users', "emaill = ?", array($data['email'])) > 0) 
			{
				...
				R::exec( "UPDATE users SET 
					...
					activation = '$activation' 
					 WHERE card_id = '$card_id' " );
 						...
 						mail($data['email'], "Регистрация в бонусной программе TheQueenStudio", "Пройдите по ссылке, чтобы активировать свою учетную запись и начать получать скидки

 					http://theqs.ru/act.php?login=$login&key=$activation
			...
			<p id="myBtn">Еще нет бонусной карты?</p>
					<div id="myModal" class="modal">
						<div class="modal-content">
							...
							<form action="newcardplease.php" method="POST">
							<p>Введите номер телефона:</p>
							<input type="number" name="contact"><br><br>
						<!--<p>Как к вам можно обращаться?:</p>
							<input type="text" name="contactname"><br><br> спасти от sql иньекции--> 
	...
	<div class="login-html">
		...
		<div class="login-form">
			...
			<div class="sign-up-htm">
				...
				<div class="group">
					<label for="user" class="label">Ваш логин</label>
					<input id="user" type="text" class="input" name="login" value="<?php echo @$data['login']; ?>">
				</div>
				<div class="group">
					<label for="email" class="label">Ваш email</label>
					<input id="user" type="email" class="input" name="email" value="<?php echo @$data['email']; ?>">
				</div>
				<div class="group">
